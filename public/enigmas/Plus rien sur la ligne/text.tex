...
\begin{center}
    \includegraphics[width=0.4\textwidth]{enigmes/Plus rien sur la ligne/image.jpg}
\end{center}
\subsection*{Énoncé}
...
\textbf{Questions} :
\begin{enumerate}
    \item Résolvez le jeu en utilisant les deux actions autorisées :
\[
A\triangleq(1,3,1,0,0) \longleftrightarrow (0,2,1,1,2) \quad \text{et} \quad B\triangleq(0,0) \longleftrightarrow (1,1).
\]

\item  Montrez que, pour que le jeu soit résoluble, il est nécessaire qu'une des actions autorisées ait un côté constitué uniquement de zéros.

\item Avec les deux actions
\[
...
\item On suppose que les coefficients constants des polynômes associés aux actions autorisées 
...
\item On suppose maintenant qu'il y a exactement deux actions, 
...
\item \emoji{hot-pepper} Soient \(P\) et \(Q\) deux polynômes de $\mathbb Z[X]$ dont les coefficients sont croissants,  
...
\item On considère une grille infinie de cases indexées par les paires \((i,j) \in \mathbb{Z} \times \mathbb{Z}\).  
...
\item Avec les deux actions (de taille non nécessairement égale)
\[
A \triangleq 
\begin{bmatrix}
0 & 0 &\cdots & 0 \\
